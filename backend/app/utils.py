...
class Parsing:
    ...
    def parse_exclude_bad(value: str) -> bool:
        ...
        Returns True to exclude bad records (filter to is_bad=0), False to include them.
        """
        ...
        if v in ("0", "false", "no"):
            return False
        # Explicit true/1/yes -> exclude; any other value (unknown) also defaults to exclude
        return True
...
class MetricNorm:
    ...
    @staticmethod
    def canonical_metric_and_unit(metric: str, unit: str) -> Tuple[str, str, bool, bool]:
        """
        ...
        if m in {"energy", "energy_total", "energy_kwh_total"}:
            if u in ENERGY_UNITS_KWH:
                return "energy_kwh_total", "kWh", False, False
            if u in ENERGY_UNITS_WH:
                return "energy_kwh_total", "kWh", True, False
            return "energy_kwh_total", unit, False, True
        return m, u, False, False
    ...
    def build_metric_condition(metric: str, include_legacy_bad: bool) -> str:
        """
        Build SQL fragment for metric filter. When include_legacy_bad and metric is
        energy_kwh_total, includes legacy rows (metric='energy' AND is_bad=1).
        """
...
class IngestUtils:
    ...
    def compute_dedupe_key(
        ...
        """Compute deterministic dedupe key for a reading; use explicit_key if provided."""
        if explicit_key:
            return explicit_key
...
# ---------------------------------------------------------------------------
class FilterBuilder:
    """Build SQL filter clauses and params for timeseries/aggregated/sum_deltas."""
    ...
    def build_aggregated_filter(
        ...
        if metric is not None:
            if include_legacy_bad_energy:
                filter_parts.append(
                    f"AND ({prefix}metric = :metric OR ({prefix}metric = 'energy' AND {prefix}is_bad = 1))"
                )
            else:
    ...
    def build_time_filter(
        ...
        """Build time-range filter clause and params (e.g. for bad_points query). Returns (time_filter, params)."""
        prefix = table_alias + "." if table_alias else ""
        parts = []
        params = {}
        if start is not None:
...
class Mappers:
    ...
    def row_to_measurement(r: Any) -> Measurement:
        ...
        return Measurement(
            ...
            raw_event_id=int(r["raw_event_id"]) if r.get("raw_event_id") is not None else None,
        )
